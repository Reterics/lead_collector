...
export default abstract class DBModel {
    ...
    async loadPersisted() {
        ...
        try {
            cache = {
                questionnaires: (await loadFromIndexedDB('questionnaires')) as CommonCollectionData[],
    ...
    async savePersisted() {
        try {
            await saveToIndexedDB('questionnaires', this._cache['questionnaires']);
