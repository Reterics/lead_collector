      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Link to="/" className="inline-flex items-center gap-2 text-blue-600 hover:underline" title={t('submissions.back')}>
              <FiArrowLeft />
              <span>{t('submissions.back')}</span>
            </Link>
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">{t('submissions.title')}</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={refresh} className="inline-flex items-center gap-2 px-3 py-2 text-sm rounded-md bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white" title={t('submissions.refresh')}>
              <FiRefreshCcw />
                  ...
                  <div>
                    ...
                    {s.issueKey && s.issueUrl && (
                      <div className="mt-1 text-sm">
                        <a href={s.issueUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-blue-600 hover:underline" title={t('submissions.openInJira')}>
                          <FiExternalLink />
                    ...
                    <details className="mt-2">
                      <summary className="text-xs text-gray-500 cursor-pointer">{t('submissions.details')}</summary>
                      <pre className="mt-1 text-xs whitespace-pre-wrap text-gray-700 dark:text-gray-300">{s.description}</pre>
                    </details>
                  </div>
                  <div>
                    <button onClick={() => onDelete(s.id)} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md bg-red-600 text-white hover:bg-red-700" title={t('submissions.delete')}>
                      <FiTrash2 />
                      <span className="hidden sm:inline">{t('submissions.delete')}</span>
                    </button>
                  </div>
                </div>
              </div>
