      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button onClick={refresh} className="px-3 py-2 text-sm rounded-md bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white">Refresh</button>
          </div>
        </div>
        ...
        {sorted.length === 0 ? (
          ...
          <div className="space-y-3">
            {sorted.map((s) => (
              <div key={s.id} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    {s.issueKey && s.issueUrl && (
                      <div className="mt-1 text-sm">
                        <a href={s.issueUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">
                          {s.issueKey}
                        </a>
                      </div>
                    )}
                  ...
                  <div>
                    <button onClick={() => onDelete(s.id)} className="px-3 py-2 text-sm font-medium rounded-md bg-red-600 text-white hover:bg-red-700">Delete</button>
                  </div>
                </div>
              </div>
            ))}
