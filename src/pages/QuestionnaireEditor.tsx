    ...
    <section className="min-h-screen bg-gray-50 dark:bg-gray-900 py-6">
      <div className="max-w-3xl mx-auto px-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">{existing ? t('editor.headingEdit') : t('editor.headingCreate')}</h2>
          <div className="flex items-center gap-2">
            <Link to="/" className="inline-flex items-center gap-2 text-blue-600 hover:underline" title={t('app.back')}>
              <FiChevronLeft />
        ...
        <div className="space-y-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div>
            <label className="block text-sm font-medium text-gray-800 dark:text-gray-200">{t('editor.name')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 dark:text-gray-200">{t('editor.description')}</label>
          ...
          {questions.map((q, idx) => (
            <div key={q.id} className="group relative overflow-hidden bg-gradient-to-br from-white to-slate-50 dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t('editor.question', { index: idx + 1 })}</h3>
                <div className="flex gap-2">
                  <button onClick={() => onDupe(q.id)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200" title={t('editor.duplicate')}>
                    <FiCopy />
                    <span className="hidden sm:inline">{t('editor.duplicate')}</span>
                  </button>
                  <button onClick={() => onRemoveQuestion(q.id)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-red-600 text-white" title={t('editor.remove')}>
                    <FiTrash2 />
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="block text-xs text-gray-700 dark:text-gray-300">{t('editor.label')}</label>
                ...
                <div>
                  <label className="block text-xs text-gray-700 dark:text-gray-300">{t('editor.type')}</label>
              ...
              <div className="mt-3">
                <label className="block text-xs text-gray-700 dark:text-gray-300">{t('editor.descriptionOptional')}</label>
              ...
              {(q.type === 'radio' || q.type === 'dropdown') && (
                <div className="mt-3">
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300">{t('editor.options')}</label>
                    <button onClick={() => onAddOption(q.id)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-blue-600 text-white" title={t('editor.addOption')}>
                      <FiPlus />
                      <span className="hidden sm:inline">{t('editor.addOption')}</span>
                    </button>
                  </div>
                    ...
                    {(q.options ?? []).map((opt, i) => (
                      <div key={i} className="flex gap-2">
                        ...
                        <button onClick={() => onRemoveOption(q.id, i)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-red-600 text-white" title="Remove option">
                          <FiTrash2 />
                          <span className="hidden sm:inline">{t('editor.removeOption')}</span>
                        </button>
                      </div>
                    ))}
                  </div>
        ...
        <div className="mt-4 flex gap-2">
          <button onClick={onAddQuestion} className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white" title={t('editor.addQuestion')}>
            <FiPlus />
            <span>{t('editor.addQuestion')}</span>
          </button>
          <button onClick={onSave} disabled={!canSave} className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-emerald-600 text-white disabled:opacity-60" title={t('editor.save')}>
            <FiSave />
